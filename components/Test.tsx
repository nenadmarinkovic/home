import React from 'react';
import TestChart from './TestChart';
import Container from '@/containers/Container';

import boxStyles from '../styles/components/Box.module.css';

function Test() {
  return (
    <Container>
      <div
        className={`${boxStyles.box_container} ${boxStyles.box_second}`}
      >
        <div className={boxStyles.box_detail}>
          <div className={boxStyles.box_charts}>
            <div className={boxStyles.box_chart_items}>
              <TestChart
                maxValue={99}
                circleValue={0.97}
                testChartText="Performance"
              />
              <TestChart
                maxValue={100}
                circleValue={1}
                testChartText="Accessibility"
              />
            </div>
            <div className={boxStyles.box_chart_items}>
              <TestChart
                maxValue={100}
                circleValue={1}
                testChartText="Best Practices"
              />
              <TestChart
                maxValue={100}
                circleValue={1}
                testChartText="SEO"
              />
            </div>
          </div>
        </div>
        <div className={boxStyles.box_item}>
          <div className={boxStyles.box_item_inside}>
            <h4
              className={`${boxStyles.box_item_header_title} ${boxStyles.green}`}
            >
              Test
            </h4>
            <p className={boxStyles.box_item_header_text}>
              better P99 latency and more requests per second than
              Apollo Router
            </p>
            <p
              className={`${boxStyles.box_item_main_title} ${boxStyles.green}`}
            >
              Deliver raw performance with the Cosmo Router
            </p>
            <div className={boxStyles.box_item_main_items}>
              <div className={boxStyles.box_item_main_item}>
                <svg
                  className={boxStyles.box_item_main_icon}
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth="1.5"
                  stroke="currentColor"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M15.59 14.37a6 6 0 0 1-5.84 7.38v-4.8m5.84-2.58a14.98 14.98 0 0 0 6.16-12.12A14.98 14.98 0 0 0 9.631 8.41m5.96 5.96a14.926 14.926 0 0 1-5.841 2.58m-.119-8.54a6 6 0 0 0-7.381 5.84h4.8m2.581-5.84a14.927 14.927 0 0 0-2.58 5.84m2.699 2.7c-.103.021-.207.041-.311.06a15.09 15.09 0 0 1-2.448-2.448 14.9 14.9 0 0 1 .06-.312m-2.24 2.39a4.493 4.493 0 0 0-1.757 4.306 4.493 4.493 0 0 0 4.306-1.758M16.5 9a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0Z"
                  />
                </svg>
                <span className={boxStyles.box_item_main_item_text}>
                  Native High-Performance Query Execution Planner
                  written in Go with Caching.
                </span>
              </div>
              <div className={boxStyles.box_item_main_item}>
                <svg
                  className={boxStyles.box_item_main_icon}
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth="1.5"
                  stroke="currentColor"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M15.59 14.37a6 6 0 0 1-5.84 7.38v-4.8m5.84-2.58a14.98 14.98 0 0 0 6.16-12.12A14.98 14.98 0 0 0 9.631 8.41m5.96 5.96a14.926 14.926 0 0 1-5.841 2.58m-.119-8.54a6 6 0 0 0-7.381 5.84h4.8m2.581-5.84a14.927 14.927 0 0 0-2.58 5.84m2.699 2.7c-.103.021-.207.041-.311.06a15.09 15.09 0 0 1-2.448-2.448 14.9 14.9 0 0 1 .06-.312m-2.24 2.39a4.493 4.493 0 0 0-1.757 4.306 4.493 4.493 0 0 0 4.306-1.758M16.5 9a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0Z"
                  />
                </svg>
                <span className={boxStyles.box_item_main_item_text}>
                  Native High-Performance Query Execution Planner
                  written in Go with Caching.
                </span>
              </div>
              <div className={boxStyles.box_item_main_item}>
                <svg
                  className={boxStyles.box_item_main_icon}
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth="1.5"
                  stroke="currentColor"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M15.59 14.37a6 6 0 0 1-5.84 7.38v-4.8m5.84-2.58a14.98 14.98 0 0 0 6.16-12.12A14.98 14.98 0 0 0 9.631 8.41m5.96 5.96a14.926 14.926 0 0 1-5.841 2.58m-.119-8.54a6 6 0 0 0-7.381 5.84h4.8m2.581-5.84a14.927 14.927 0 0 0-2.58 5.84m2.699 2.7c-.103.021-.207.041-.311.06a15.09 15.09 0 0 1-2.448-2.448 14.9 14.9 0 0 1 .06-.312m-2.24 2.39a4.493 4.493 0 0 0-1.757 4.306 4.493 4.493 0 0 0 4.306-1.758M16.5 9a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0Z"
                  />
                </svg>
                <span className={boxStyles.box_item_main_item_text}>
                  Native High-Performance Query Execution Planner
                  written in Go with Caching.
                </span>
              </div>
            </div>
          </div>
          <div
            className={`${boxStyles.blur} ${boxStyles.blur_green}`}
          ></div>
        </div>
        <div className={boxStyles.box_dots}></div>
      </div>
    </Container>
  );
}

export default Test;
